...
class ComputersControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create computer" do
    assert_difference('Computer.count') do
      post computers_url, params: { computer: { hostname: @computer.hostname, status: @computer.status } }
    end
  ...
  test "should update computer" do
    patch computer_url(@computer), params: { computer: { hostname: @computer.hostname, status: @computer.status } }
